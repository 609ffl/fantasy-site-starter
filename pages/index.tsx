...
export default function Home() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        (tRes.teams || []).forEach((t: Team) => { map[t.id] = t; });
        setTeams(map);
        setGames(gRes.games || []);
      } catch (e: any) {
  ...
  const name = (id: number | null) => (id && teams[id]) ? teams[id].name : (id ? `Team ${id}` : "—");
  ...
  return (
    <main style={{ maxWidth: 960, margin: "40px auto", padding: 16, fontFamily: "system-ui, sans-serif" }}>
      ...
      {loading && <p>Loading…</p>}
      {err && <pre style={{ background: "#fee", padding: 12, border: "1px solid #f99" }}>{err}</pre>}
      {!loading && !err && games.length === 0 && <p>No games for week {week}.</p>}
